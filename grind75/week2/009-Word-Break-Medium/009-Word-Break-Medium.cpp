...
// Approach:
// Start at the empty string (dp[0]) and find the words that can be built from there.
// Mark the ending index as true in the dp array. 
// If at the end, that index (dp[s.length()]) is true, it means the string can be built
